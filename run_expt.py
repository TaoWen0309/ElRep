...
parser.add_argument('--save_last', type=parse_bool, const=True, nargs='?', default=False)
parser.add_argument('--save_pred', type=parse_bool, const=True, nargs='?', default=False)
...
if __name__=='__main__':
    ...
    config.log_dir = os.path.join(config.log_dir, f"{config.dataset}", f"{config.seed}", f"theta1-{config.theta1}_theta2-{config.theta2}")
